...
#		print "LOADING $f\n";
	...
	function parse_map_entry($e){
	   if($e[0]=="#") return;
		...
		$p=array_shift($r); $a=array_shift($r);
		$this->map[$p]=$a;
...
#      log_error($path);
		...
		foreach($this->map as $p=>$a){
			...
			$path_r=str_replace(array('$controller', '$action'), 
				array("([_\d\w]+)","([_\d\w]+)"), $p);
				
			// parameter regex
			if(isset($this->map_regex[$p]))
				$path_r=str_replace(array_keys($this->map_regex[$p]),
					array_values($this->map_regex[$p]), $path_r);
...
#         print "P: $proto; H: $host\n";
      ...
      $caL=explode("/", $action);
      $act=array_pop($caL);
      if(@XorcApp::$inst->conf['general']['routingbug']){
         $cont=join("/", $caL);
      }else{
         if(isset($caL[1])){
            $prefix=$caL[0];
...
#    			log_error("VGL {$a} vs. $action");
    			if($a==$action){
    				$url=str_replace(array('$controller', '$action'), 
    					array($cont, $act), $p);
    				break;
    			}
    		}
