...
class JQuery extends JavaScript  {
		...
		public $CALLBACKS 	=  	['beforeSend', 'complete', 'error', 'success'];
		public $CONSTANTS =    ['hide', 'show', 'toggle'];
	...
	{
		$options['form'] = true;
		return '<form action="'.$options['url'].'" onsubmit=\''.$this->remote_function($options).'; return false;\' method="'.($options['type'] ?? 'GET').'"  >';			
	}
	...
	function visual_effect($name,$element,$options=null) {
		...
		switch($name) {
			case 'animate'	:
				$params = $this->_options_for_javascript($options,['hide', 'show', 'toggle']);
	...
	function observe_field($field_id,$options=null)
	{
		if (isset($options['frequency']) && $options['frequency']> 0 ) {
			return $this->_build_observer($field_id, false,$options);
		} else {
			return $this->_build_observer($field_id, true,$options);
		}
	}
	...
	function _build_observer($name, $event=false,$options=null)
	{
	...
	{
		return (strstr((string) $method,"'"))?$method:"'$method'";
	}
  ...
  if (isset($options['url'])) $js_options['url']    = '"'.$options['url'].'"';
		...
		if (isset($options['form'])) {
			$js_options['data']='$(this).formSerialize()';		
		}elseif (isset($options['parameters'])){
			$js_options['data']='$("'.$options['submit'].'").formSerialize()';
		}elseif (isset($options['with'])) {
			$js_options['data']= '"'.$options['with'].'"';
		}
		
		$html_update=($options['position'] ?? 'html');
		if (isset($options['update']))$options['success']='$("'.$options['update'].'").'.$html_update.'(response);'.($options['success'] ?? '');
				
		$js_options=array_merge($js_options,(is_array($options))?$this->_build_callbacks($options):[]);
