<?php
class Xorc_objectform {
   var $obj = null;
   var $name = null;
   var $opts;

   function __construct($o, $oname, $opts = []) {
      $default = ['autoid' => 1, 'canonical_classnames' => false, 'req' => false, 'modifier' => null];
      $this->obj = $o;
      $this->name = $oname;
...
*/
   ...
   function value($name, $opts = []) {
      ...
      $val = $this->obj->$name;
      if (($opts['modifier'] ?? null) && is_callable($opts['modifier'])) {
   ...
   function default_options($name, $opts = [], $val = null) {
      $default = ['req' => false];
      # achtung! opts kann auch wiederum arrays enthalten
      $nopts = array();
      foreach ($opts as $k => $v) {
         // filter null values ex: id => null
      ...
      if ($this->opts['autoid']) {
         $default['id'] = $this->idname($name);
         if (!is_null($val)) $default['id'] .= "_$val";
      }
      //var_dump($opts);
      //var_dump($default);
      //var_dump($nopts);
      return array_merge($default, $nopts);
   }
