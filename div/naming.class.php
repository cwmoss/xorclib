...
class Naming{
    ...
    var $uncountable_words = array("equipment", "information", "rice", "money",
		"species", "series", "fish" );

    var $plural_rules = array(
        array('/^(ox)$/i', '\1\2en'),  		          // ox
	...
	function singular($t){
		$t=strtolower(trim($t));
		if(array_search($t, $this->uncountable_words)){
			return $t;
		}
		foreach($this->singular_rules as $r){
			if(preg_match($r[0], $t)){
	...
	function plural($t){
		...
		foreach($this->plural_rules as $r){
