...
$_DEBUGLEVEL=0;
function dump($var='', $msg='', $level=''){
	global $_DEBUGLEVEL;
	if($_DEBUGLEVEL){
		if(!$var){
			print("<br>DUMPMSG: <b>$msg</b><br>");
		}else{
			print("<br>DUMPVAR: <b>$msg</b> <pre>"); print_r($var); print("</pre><br>");
		}
	}
}
...
/*
	make urlencoded http get string from hash array
*/
function GET($vars, $url=""){
	$ret = trim($url);
	foreach($vars as $k=>$v){
		$ret.=($c)?"?":"&";
...
function getNumArchPath($id){
	return sprintf("/%03d/%03d/%s", ($id/100000), $id/100, $id);
}

/*
returns PATH for different objects $prop musthave
*/
function getFileNames($prop, $i = ""){
		global $_dk, $_site;
		$ret = array();
		if(!$prop["id"]) return $ret;
		$path = sprintf("%s*", getNumArchPath($prop["id"]));
		echo "local::".$_site->getLocal($prop["prodmodul"]);
		$ret["version"] = $prop["version"];
		if(!isset($ret["version"]))$ret["version"] = 1;
		if($i) $i2="_$i";
		$ret["filename$i"] = sprintf("%s%s%sv%s%s",
			$_site->getLocal($prop["prodmodul"]),
			getNumArchPath($prop["id"]),
			$i2, $prop["version"],
			$prop["filetype"]);

		$ret["fileurl$i"] = sprintf("%s%s%sv%s%s",
			$_site->getUrl($prop["prodmodul"]),
			getNumArchPath($prop["id"]),
			$i2, $ret["version"], $prop["filetype"]);
		return $ret;
	}

function imtab($arr){ 

		$t = "<table align=\"left\" hspace=5 vspace=5>";
		for($i=0; $i<sizeof($arr); $i=$i+4){
		$u = $i;
		if(sizeof($arr)-$i == 4){
			$t .= "<tr><td><img src='/alotta/user/platoondvd/img/{$arr[$u]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td>";
			$t .= "<td><img src='/alotta/user/platoondvd/img/{$arr[$u++]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td></tr>";
			$t .= "<tr><td><img src='/alotta/user/platoondvd/img/{$arr[$u++]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td>";
			$t .= "<td><img src='/alotta/user/platoondvd/img/{$arr[$u++]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td></tr> ";
			break;
		}elseif(sizeof($arr)-$i == 3){
			$t .= "<tr><td><img src='/alotta/user/platoondvd/img/{$arr[$u]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td>";
			$t .= "<td><img src='/alotta/user/platoondvd/img/{$arr[$u++]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td></tr>";
			$t .= "<tr><td colspan=2><img src='/alotta/user/platoondvd/img/{$arr[$u++]['picpath']}t2{$arr[$u]['id']}.jpg' border=0></td></tr>";
			break;
		}elseif(sizeof($arr)-$i == 2){
			$t .= "<tr><td><img src='/alotta/user/platoondvd/img/{$arr[$u]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td>";
			$t .= "<td><img src='/alotta/user/platoondvd/img/{$arr[$u++]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td></tr>";
			break;
		}elseif(sizeof($arr)-$i == 1){
			$t .= "<tr><td colspan=2><img src='/alotta/user/platoondvd/img/{$arr[$u]['picpath']}t2{$arr[$i]['id']}.jpg' border=0></></tr>";
			break;
		}else{
			$t .= "<tr><td><img src='/alotta/user/platoondvd/img/{$arr[$u]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td>";
			$t .= "<td><img src='/alotta/user/platoondvd/img/{$arr[$u++]['picpath']}t3{$arr[$u]['id']}.jpg' border=0></td></tr>";
