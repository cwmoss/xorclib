...
class Xorcstore_Errors Implements Countable{
   private $e=array();
   public $cols=array();
   private $base=array();
   ...
   function add($e, $msg=null){
      # var_dump($e);
      if(is_null($msg) && !($e instanceof xorcstore_error)){
         return $this->add_to_base($e);
      }
      if(!is_null($msg)){
         $e=new Xorcstore_Error($e, $msg);
      }
      $this->e[]=$e;
   ...
   function add_to_base($msg){
      $this->base[]=new Xorcstore_Error("", $msg);
   }
   
   function on($col){
      $eL=array();
      if(isset($this->cols[$col])){
         foreach($this->cols[$col] as $e){$eL[]=$e->msg;}
         return $eL;
      }
      return null;
   }
