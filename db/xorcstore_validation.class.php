...
#		log_error($this->v);
      ...
      if($this->v[$e]) foreach($this->v[$e] as $vld){
         $m=$vld[1]; $opts=isset($vld[2])?$vld[2]:array();
...
#      if(is_null($v)) return new Xorcstore_Error($col, sprintf(self::$msg['empty'], $col));
      ...
      if($opts['is'] && $len!=$opts['is']){
         $msg=$opts['msg_wrong_length']?$opts['msg_wrong_length']:
            ($opts['msg']?$opts['msg']:(self::$msg['wrong_length']));
         return new Xorcstore_Error($col, sprintf($msg, $opts['name'], $opts['is']));
      }elseif(!is_null($min) && ($len<$min || is_null($v))){
         $msg=$opts['msg_too_short']?$opts['msg_too_short']:
            ($opts['msg']?$opts['msg']:(self::$msg['too_short']));
         return new Xorcstore_Error($col, sprintf($msg, $opts['name'], $min));
      }elseif(!is_null($max) && ($len>$max || is_null($v))){
         log_error( "IS NULL? ");
         log_error(is_null($v));
         $msg=$opts['msg_too_long']?$opts['msg_too_long']:
            ($opts['msg']?$opts['msg']:(self::$msg['too_long']));
   ...
   function validates_acceptance_of($e, $obj, $col, $opts=array()){
      $opts=array_merge(array("msg"=>self::$msg['accepted'], 
         'accept'=>1, "name"=>ucfirst($col)), $opts);
      if(!$this->test_if($e, $obj, $col, $opts)) return true;
      $v=$obj->$col;
      if(is_null($v)){
   ...
   function validates_uniqueness_of($e, $obj, $col, $opts=array()){
      ...
      if($opts['scope']){
         ...
         foreach($opts['scope'] as $scope){$scopeL[$scope]=$obj->$scope;}
      }
      
      if(!$obj->is_new_record()){$scopeL[]="id != ".$obj->id_quoted();}
