...
/*
function autoloader_pear($clas) {
	$try = str_replace('_', '/', (string) $clas) . ".php";
	@include_once($try);
...
*/
/*
function autoloader_xorc($clas) {
	$clas = strtolower((string) $clas);
	$path = XorcApp::$inst->include_path;
	// print $clas;
	//if($class[0]=="m") print "<pre>";print_r(debug_backtrace());print "</pre";
	// print "#$clas~";
	//  if(preg_match("/^vq/i", $clas))
	...
	if (file_exists($try)) {
		...
		$path = XorcApp::$inst->base . "/controller";
		include_once("$path/" . $clas . ".class.php");
		return true;
	}
	return false;
...
The mere presence of the __autoload function definition, even inside the conditional, seems to raise this warning on PHP 7.2.

if(!function_exists('spl_autoload_register')){
   function __autoload($clas){
      Xorc_Autoloader::load($clas);
   }
}
*/
/*
function xorc_shutdown() {
...
*/
# register_shutdown_function("xorc_shutdown");
